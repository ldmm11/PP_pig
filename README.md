...
#### 6?? MySQL ????
`ash
# MySQL 8.0 ?? caching_sha2_password?FastAPI ???????
...
| docker exec -it emotion-chat-db mysql -uroot -p1234 | ?? MySQL |

### ?? ????

1. **Docker ??????** ? ????????????????/????
2. **???????** ? ?????????? ? ???????
3. **MySQL ????** ? MySQL 8.0 ????????? mysql_native_password
4. **API Key ???** ? .env ????? docker-compose.yml ????
5. **Docker Compose ??** ? docker-compose(v1) ??????????? docker compose(v2 ??) ???
...
| ???? | ?? | ?? |
|---------|------|------|
| ?????Vue/????? | docker compose up -d --build frontend | ~10? |
| ?????Python/API? | docker compose up -d --build backend | ~30? |
| ????? | docker compose up -d --build | ~40? |
| ????????? | docker compose restart backend | ~2? |

> MySQL ??? .env ???? Docker volume ???????????
...
`ash
# ? vim ????????????
vim /www/pp_pig/backend/app/xxx.py
...
cp .env ../
# ???????????
rm -rf backend frontend nginx.conf
# ??????????
...
# ????
docker exec -it emotion-chat-backend /bin/bash   # ??????
docker exec -it emotion-chat-db mysql -uroot -p1234  # ?? MySQL
...
# ????????
tar -czf pp_pig_backup.tar.gz /www/pp_pig/docker-compose.yml /www/pp_pig/.env /www/pp_pig/nginx.conf
`
